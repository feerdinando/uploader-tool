...
function obterPermissoesPastasUsuario($userId) {
    $permissions = carregarPermissoes();
    $userConfig  = $permissions['users'][$userId] ?? null;
    ...
    // Retorna apenas as permissões (true/false como string)
    $permissoes = [];
    foreach ($userConfig['folders'] as $nome => $valor) {
        $permissoes[$nome] = $valor ? "true" : "false";
    }

    debugLog("Permissões obtidas para usuário $userId", [
        'raw_folders' => $userConfig['folders'],
...
try {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'login') {
        ...
        if ($success && $authData) {
            ...
            $pastasDoUsuario = $userId ? obterPastasUsuario($userId) : []; // Mantém para uso interno
            $permissoesPastas = $userId ? obterPermissoesPastasUsuario($userId) : []; // Nova função
            
            $authData['folders']         = $permissoesPastas;
            $authData['folders_count']   = count($permissoesPastas);
            $authData['has_permissions'] = !empty($permissoesPastas);
